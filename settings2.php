...
/* ===== GIGACOIN + THEMES LADEN ===== */
$stmtCoins = $conn->prepare("SELECT gigacoin, active_theme, owned_themes FROM users WHERE id = ?");
$stmtCoins->bind_param("i", $user_id);
$stmtCoins->execute();
$resultCoins = $stmtCoins->get_result();
$userData = $resultCoins->fetch_assoc();
$stmtCoins->close();

$_SESSION['gigacoin'] = (int)($userData['gigacoin'] ?? 0);
$_SESSION['active_theme'] = $userData['active_theme'] ?? 'default';
$_SESSION['owned_themes'] = $userData['owned_themes'] ?? '';

if (isset($_POST['activate_theme']) && $_POST['activate_theme'] === 'default') {
    ...
    $_SESSION['active_theme'] = 'default';

    header("Location: index.php");
    exit;
}

if (isset($_POST['buy_theme'])) {

    $theme = $_POST['buy_theme'];

    if (!isset($themePrices[$theme])) {
        exit;
    }

    $price = $themePrices[$theme];

    $owned = explode(',', $_SESSION['owned_themes']);

    // Wenn schon gekauft → nur aktivieren
    if (in_array($theme, $owned)) {
    ...
    // Noch nicht gekauft
    if ($_SESSION['gigacoin'] >= $price) {

        $owned[] = $theme;
        $newOwned = implode(',', array_filter($owned));

        $stmt = $conn->prepare("
            UPDATE users
            SET gigacoin = gigacoin - ?,
                active_theme = ?,
                owned_themes = ?
            WHERE id = ?
        ");
        $stmt->bind_param("issi", $price, $theme, $newOwned, $user_id);
        $stmt->execute();
        $stmt->close();

        $_SESSION['gigacoin'] -= $price;
        $_SESSION['active_theme'] = $theme;
        $_SESSION['owned_themes'] = $newOwned;

        header("Location: index.php");
        exit;
    }

    header("Location: index.php?shop=notenough");
    exit;
}
// CSRF Token erzeugen
    ...
    <div class="settings-header">
      ...
      <div class="layout">
        ...
        <section>
          ...
          <div id="shopMenu" class="shop-menu" style="display:none;">
              <h3>Themes</h3>

              <form method="POST">

                  <button type="submit" name="activate_theme" value="default">
                      ⚪ Default aktivieren
                  </button>

                  <?php $owned = explode(',', $_SESSION['owned_themes']); ?>

                  <button type="submit" name="buy_theme" value="purple">
                      <?php if (in_array('purple', $owned)) { ?>
                          🌙 Purple aktivieren
                      <?php } else { ?>
                          🌙 Purple Theme (50 Coins)
                      <?php } ?>
                  </button>

                  <!-- OCEAN -->
                  <button type="submit" name="buy_theme" value="ocean">
                  <?php if (in_array('ocean', $owned)) { ?>
                  🌊 Ocean aktivieren
                  <?php } else { ?>
                  🌊 Ocean Theme (40 Coins)
                  <?php } ?>
                  </button>


                  <!-- FOREST -->
                  <button type="submit" name="buy_theme" value="forest">
                  <?php if (in_array('forest', $owned)) { ?>
                  🌲 Forest aktivieren
                  <?php } else { ?>
                  🌲 Forest Theme (40 Coins)
                  <?php } ?>
                  </button>


                  <!-- SUNSET -->
                  <button type="submit" name="buy_theme" value="sunset">
                  <?php if (in_array('sunset', $owned)) { ?>
                  🌅 Sunset aktivieren
                  <?php } else { ?>
                  🌅 Sunset Theme (70 Coins)
                  <?php } ?>
                  </button>


                  <!-- CYBER -->
                  <button type="submit" name="buy_theme" value="cyber">
                  <?php if (in_array('cyber', $owned)) { ?>
                  ⚡ Cyber aktivieren
                  <?php } else { ?>
                  ⚡ Cyber Theme (120 Coins)
                  <?php } ?>
                  </button>

                    <!-- MIDNIGHT CHERRY -->
                  <button type="submit" name="buy_theme" value="midnight_cherry">
                  <?php if (in_array('midnight_cherry', $owned)) { ?>
                  🍒 Midnight Cherry aktivieren
                  <?php } else { ?>
                  🍒 Midnight Cherry Theme (120 Coins)
                  <?php } ?>
                  </button>

                  <!-- AURORA BOREALIS -->
                  <button type="submit" name="buy_theme" value="aurora_borealis">
                  <?php if (in_array('aurora_borealis', $owned)) { ?>
                  🌌 Aurora Borealis aktivieren
                  <?php } else { ?>
                  🌌 Aurora Borealis Theme (120 Coins)
                  <?php } ?>
                  </button>
