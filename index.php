...
/* ===== EIGENER GIGASCORE ===== */
$stmt = $conn->prepare("SELECT gigascore FROM users WHERE id = ?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$myScore = (int)($stmt->get_result()->fetch_assoc()['gigascore'] ?? 0);
$stmt->close();
        ...
        <h2 id="chatWith" class="chat-header clickable-group">
          <span class="chat-left">
            Chat mit: <span id="chatWithName">
              <?php echo htmlspecialchars($currentUser['username'] ?? 'Unbekannt'); ?>
            </span>
...
const SESSION_ROLE = <?= (int)($_SESSION['role'] ?? 0) ?>;
    ...
    .then(data => {
      ...
      data.forEach(msg => {
        ...
        if (msg.deleted == 1) {
          ...
          content = `<img src="${msg.gif_url}" class="chat-gif">`;
        }
        else if (msg.is_image == 1 && msg.file_path) {
          content = `<img src="${msg.file_path}" class="chat-image" data-full="${msg.file_path}">`;
        }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function switchChat(el) {
      ...
      window.lastDay = null;
        ...
        // letzten Chat merken (30 Tage)
        document.cookie = "last_chat_user=" + currentChat + "; max-age=" + (60*60*24*30) + "; path=/";
