...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!preg_match('/^[A-Za-z0-9_.-]{3,30}$/', $username)) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            // 1. normaler Login mit Klartext-Passwort
            if (password_verify($password, $storedHash)
                // 2. ODER: der User gibt direkt den Hash als Passwort ein
                ...
                // remember-me bleibt wie gehabt ...
                if (isset($_POST['remember']) && $_POST['remember'] == '1') {
                    $selector   = bin2hex(random_bytes(8));
                    $token      = random_bytes(32);
                    $hashedToken = password_hash($token, PASSWORD_DEFAULT);
                
                    $stmt_rm = $conn->prepare("INSERT INTO remember_tokens (user_id, selector, hashed_token, expires_at)
                                               VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 30 DAY))
                                               ON DUPLICATE KEY UPDATE hashed_token = VALUES(hashed_token), expires_at = VALUES(expires_at)");
...
<style>
    ...
    /* ======== Mobile Anpassungen ======== */
    @media (max-width: 768px) { /* iPad Hoch- oder Querformat */
        form {
            min-width: 90%; /* fast volle Breite */
            padding: 30px;
            gap: 15px;
        }
        input, button {
    ...
    @media (max-width: 480px) { /* iPhone */
        form {
            min-width: 95%; /* fast volle Breite */
            padding: 25px;
            gap: 12px;
        }
        input, button {
            padding: 20px;
            font-size: 22px; /* extra groß */
        }
    }
</style>